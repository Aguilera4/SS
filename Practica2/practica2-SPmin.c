...
int main(int argc, char *argv[]){
      ...
      distribucion = 1;
        ...
        printf("\nFormato Argumentos -> <x> <y> <z> <num_s> <veces> <distribucion>\n");
   		exit(1);
  	}
  	else{
    	sscanf(argv[1],"%d",&x);
    	sscanf(argv[2],"%d",&y);
      sscanf(argv[3],"%d",&z);
    	sscanf(argv[4],"%d",&num_s);
    	sscanf(argv[5],"%d",&veces);
    	sscanf(argv[6],"%d",&distribucion);
  	}
