...
vector<float> mediasSimulaciones;
...
void fin_atraque()
{
    ...
    ///////////// MODIFICADO ////////////////
    acum_toneladas += devuelve_carga(nodo.reg_cola.tipo);
...
void fin_simulacion()
{
  int i;
    ...
    // Acumulamos los valores de NBCA
    mediasSimulaciones[cont_simu] += acum_lleg/reloj;
...
int devuelve_carga(int tipo){
  if(tipo==0) return peso_tipo0;
  else if(tipo==1) return peso_tipo1;
  else return peso_tipo2;
}
...
{
  int i, simulaciones, vueltas;
  
  if(argc != 3)
    {
    printf("\n\nFormato Argumentos -> <numero_simulaciones numero_vueltas_por_simulacion>\n\n");
    exit(1);
  }
  
  sscanf(argv[1],"%d",&simulaciones);
  sscanf(argv[2],"%d",&vueltas);

  // Establecemos el viaje del remolcador cuando esta vacío 
  // desde la bocana a los puntos de atraque y viceversa
  // a 0.15
  tviajevacio = 0.15;
  ...
  for(i=0; i<simulaciones; i++){
    mediasSimulaciones.push_back(0.0);
  }
  ...
  {
    int cont_vueltas;
    for(cont_vueltas=0; cont_vueltas<vueltas; cont_vueltas++){
  ...
  int j;
  for(j=0; j<simulaciones; j++){
    printf("%f\n", mediasSimulaciones[j]/vueltas);
  }
